...
def CheckVcf(refPath, project):
    if os.path.exists('ref_data/'+project+'.somatic.vcf.gz') is False:
        pass
    

#if (-e "$ref_dir/$project.somatic.vcf.gz"){
#   if (-z "$ref_dir/$project.somatic.vcf.gz"){system("echo $data_dir/$project >> $main_dir/$data_dir\_bad_report_hg19.txt");} else{
#   system("mv $ref_dir/$project* $store_dir/$project");
#   system("rm -rf $rawdata_dir/*/$fq1_name_N.fq.gz $rawdata_dir/*/$fq2_name_N.fq.gz $rawdata_dir/*/$fq1_name_T.fq.gz $rawdata_dir/*/$fq2_name_T.fq.gz");
#   system("echo $data_dir/$project >> $main_dir/$data_dir\_good_report_hg19.txt");
#   }
#}
#else{
#   system("echo $data_dir/$project >> $main_dir/$data_dir\_bad_report_hg19.txt");
#}
...
def test(n):
    print('call from NGStools, patient:%s' %(n))
    a = 'afdwa67ad'
    return a
